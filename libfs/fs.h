 ...
 * `fs_lseek(fd, fs_stat(fd));` prior to writing.
 *
 * Return: -1 if no FS is currently mounted, or if file descriptor @fd is
 ...
 * least @count bytes.
 *
 * If the current offset is within the existing file, then the function
 * overwrites existing data. Otherwise, when the function attempts to write past
 * the end of the file, the file is automatically extended to hold the
 * additional bytes. If the underlying disk runs out of space while performing a
 * write operation, fs_write() should write as many bytes as possible. The
 * number of written bytes can therefore be smaller than @count (it can even be
 * 0 if there is no more space on disk).
 *
